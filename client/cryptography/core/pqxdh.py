...
def initiate(
    ...
    if opks_kem:
        # Match the ML-KEM OPK with the same opk_id as the X25519 OPK used
        # in DH4. If no X25519 OPK was available, use the first ML-KEM OPK.
        target_id  = opk_id_used if opk_id_used is not None else opks_kem[0]["opk_id"]
        kem_opk    = next(
            (o for o in opks_kem if o["opk_id"] == target_id),
            opks_kem[0]
        )
        opk_kem_pub   = bytes.fromhex(kem_opk["opk_pub"])
        opk_id_used   = kem_opk["opk_id"]   # ensure both legs use same id
        ct_pq, ss_pq  = _kem_encapsulate(opk_kem_pub)
    ...
    # PQXDH spec §3.3: IKM = F || DH1 || DH2 || DH3 || [DH4] || SS_pq
...
def _zeroise(*bufs: bytes) -> None:
    """Best-effort zeroisation. Python cannot guarantee memory clearing."""
    for buf in bufs:
        if buf:
            try:
                del buf
