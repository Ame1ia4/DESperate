...
# verify_hybrid_signature lives in core.keys (imported above) so that
# verify_spk_signature can call it without a circular import.
...
def verify_ciphertext(
    ...
    # ── Delegate to shared primitive ──────────────────────────────────────────
    # verify_hybrid_signature raises MalformedSignedCiphertextError if
    # ik_sig_pub is the wrong length, returns False on a bad signature.
    try:
